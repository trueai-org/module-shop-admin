...
import {
    ...
    Table, Tabs, Cascader, Radio, Avatar, DatePicker, Tag, Row, Col
} from 'antd';
...
@connect()
@Form.create()
class ShipmentAdd extends PureComponent {
    constructor(props) {
        super(props);
        this.state = {
            loading: false,
    ...
    render() {
        ...
        return (
            <PageHeaderWrapper title="订单发货" action={action}>
                <Card bordered={false}>
                    <Form onSubmit={this.handleSubmit}>
                        <FormItem>
                            {getFieldDecorator('no', {
                                ...
                                size="large" placeholder="请输入订单编号" />)}
                        </FormItem>
                        <Row gutter={16}>
                            <Col span={8}>
                                    ...
                                    <FormItem>
                                        {
                                            getFieldDecorator('totalWeight', {
                                                ...
                                                <InputNumber min={0} precision={3} style={{ width: 120 }} placeholder="重量/kg" />
                                            )
                                        }
                                        <span> kg</span>
                                    </FormItem>
                                    <FormItem>
                                        {getFieldDecorator('adminComment',
                                            { initialValue: '' }
                                        )(
                                ...
                                <Spin spinning={this.state.loading}>
                                    <Card type="inner" title={`订单 ${this.state.orderStatusStr}`}>
                                        ...
                                        <div style={{ marginBottom: 12 }}>
                                            订单总额：
                                    <span style={{ fontWeight: 'bold' }}>{this.state.current.orderTotal}</span> ¥</div>
                                        <Table
                                            bordered={false}
